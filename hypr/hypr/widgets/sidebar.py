...
## methods
    ...
    def toggle_window_stopwatch(self):
        if self.stopwatch_box.is_visible():
            self.stopwatch_box.hide()
        else:
            self.stopwatch_box.show()
    ...
    def start_stopwatch(self, *_):
        if not self.stopwatch_running:
            ...
            while self.stopwatch_running:
                self.update_time(self.stopwatch_time_label.get_label())
    ...
    def reset_stopwatch(self, *_):
        self.stop_stopwatch()
        self.stopwatch_time_label.set_label("00:00")
    ...
    def add_one_second(self, time_str):
        ...
        # Handle overflow if seconds reach 60
        if seconds >= 60:
            minutes += 1
            seconds = 0

        # Return new time string, zero-padded for seconds
    ...
    def update_time(self, time_str):
        ...
        new_time = self.add_one_second(time_str)
        self.stopwatch_time_label.set_label(new_time)

    @staticmethod
    def format_stopwatch_time(seconds: float) -> str:
        h = int(seconds // 3600)
